...
func setFunc(key string, val *item ,c *Cache)  {
	...
	c.dp.queue[getHash(&key,len(c.dp.queue))] <- nc
}
...
func getHash(key *string, size int) byte {
	return (*key)[0]%uint8(size)
}

func getFunc(key string, c *Cache, ch chan interface{})  {

	c.dp.queue[getHash(&key,len(c.dp.queue))] <- newCommons(&onceGetFunc, newCommonsData(key, nil, c,ch))
}
...
func delFunc(key string ,c *Cache)  {
	c.dp.queue[getHash(&key,len(c.dp.queue))] <- newCommons(&onceDelFunc, newCommonsData(key, nil, c, nil))
}
